...
def cal_loss(model, images, labels):
    ...
    with tf.GradientTape() as tape:
        ...
        logit_distribution = tf.nn.softmax(logits, 1)
...
def main():
    model = original_GL_network(input_shape=(FLAGS.img_height, FLAGS.img_width, 1), num_classes=88)
    model.summary()
    ...
    if FLAGS.train:
        ...
        train_log_dir = FLAGS.graphs + current_time + "_V15" + '/train'
        train_summary_writer = tf.summary.create_file_writer(train_log_dir)

        val_log_dir = FLAGS.graphs + current_time + "_V15" + '/val'
